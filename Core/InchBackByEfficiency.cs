...
{
    public class InchBackByEfficiency
    {
        public Roster Run(IEnumerable<Player> players)
        {
            ...
            var allTopPlayersOrdered = players
                ...
                .OrderBy(p => p.PointPerCost).ToArray();
            ...
            while (roster.Salary > 50000)
            {
                ...
                {
                    var leastEfficientAtPosition = roster
                        .Where(p => p.Position == newPlayer.Position)
                        .OrderByDescending(p => p.PointPerCost)
                        .First();

                    roster.Remove(leastEfficientAtPosition);
